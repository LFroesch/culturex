  ...
  if (!wikiData.fullHtml) {
    return (
      <>
        ...
        <h3 className="font-bold text-xl mb-2">{wikiData.title}</h3>
        <p className="text-gray-700 mb-4">{wikiData.extract}</p>
        {wikiData.url && (
          <a href={wikiData.url} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline">
            Read on Wikipedia →
          </a>
        )}
      </>
      ...
      {sections.map((section, index) => (
        <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          >
            ...
            <svg
              className={`w-5 h-5 text-gray-500 transition-transform ${openSections.has(index) ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {openSections.has(index) && (
            <div
              className="px-4 py-3 prose prose-sm max-w-none wiki-content"
...
const CityPanel = ({ city, onClose }: CityPanelProps) => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 top-16 w-full md:w-1/2 bg-white shadow-2xl z-[1001] overflow-hidden flex flex-col">
      {/* Header */}
                ...
                {news.length > 0 ? (
                  news.map((article, idx) => (
                    <div key={idx} className="border-b border-gray-200 pb-4">
                      ...
                      <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-primary-600 text-sm hover:underline">
                        Read more →
                      </a>
                    </div>
                  ))
            ...
            {activeTab === 'history' && (
              <div>
                {loading ? (
                  ...
                  <p className="text-gray-500">{wikiData.error}</p>
                ) : wikiData ? (
            ...
            {['insight', 'photo', 'food', 'recipe', 'story', 'music', 'workExchange', 'forum'].includes(activeTab) && (
              <div className="space-y-4">
                {posts.length > 0 ? (
                  ...
                  <p className="text-gray-500">No content yet. Be the first to contribute!</p>
                )}
              </div>
            )}
          </>
